<x-filament-panels::page>
    @php
        ...
        $avatarUrl = $user?->profile_picture ? Storage::disk('public')->url($user->profile_picture) : null;
    @endphp
                ...
                <div class="rb-admin-edit2__card rb-admin-edit2__card--subtle">
                    <div class="rb-admin-edit2__avatar-wrap">
                        <div class="rb-admin-edit2__avatar">
                            @if ($avatarUrl)
                                <img src="{{ $avatarUrl }}" alt="{{ $user?->name }}" class="rb-admin-edit2__avatar-img" loading="lazy" decoding="async">
                            @else
                                <img src="https://ui-avatars.com/api/?name={{ urlencode((string) ($user?->name ?? 'Super Admin')) }}&background=4f46e5&color=fff" alt="{{ $user?->name }}" class="rb-admin-edit2__avatar-img" loading="lazy" decoding="async">
                            @endif
                    ...
                    <div class="rb-admin-edit2__name">{{ $user?->name }}</div>
                    <div class="rb-admin-edit2__email">{{ $user?->email }}</div>
