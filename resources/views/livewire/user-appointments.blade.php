<div>
                        ...
                        <div class="flex items-center gap-2">
                            @if($status === 'completed')
                                <button type="button" wire:click="bookAgain({{ $booking->id }})"
                                    ...
                                    <button type="button" wire:click="openRatingModal({{ $booking->id }})"
    ...
    @if($rateModalOpen)
        <div class="fixed inset-0 z-[999]" x-data>
            <div class="absolute inset-0 bg-slate-950/70 backdrop-blur-sm" wire:click="closeRatingModal"></div>
            <div class="absolute inset-0 flex items-center justify-center p-5">
                <div class="w-full max-w-xl overflow-hidden rounded-3xl bg-white shadow-[0_30px_90px_rgba(15,23,42,0.35)] ring-1 ring-slate-900/10 dark:bg-slate-900">
                    <div class="flex items-start justify-between gap-4 border-b border-slate-200/70 p-6 dark:border-slate-800/70">
                        <div>
                            <div class="text-lg font-extrabold text-slate-900 dark:text-white">Rate Company</div>
                            <div class="mt-1 text-sm font-medium text-slate-500 dark:text-slate-400">
                                {{ $rateCompanyName ?: 'Company' }}
                            </div>
                        </div>
                        <button type="button" class="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-100 text-slate-600 hover:bg-slate-200 transition dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700" wire:click="closeRatingModal">
                    ...
                    <div class="p-6 space-y-5">
                        <div>
                            <div class="text-sm font-extrabold text-slate-900 dark:text-white">Your rating</div>
                            <div class="mt-3 flex items-center gap-2">
                                @for($i = 1; $i <= 5; $i++)
                                    <button type="button" wire:click="$set('rateValue', {{ $i }})"
                                        class="inline-flex h-12 w-12 items-center justify-center rounded-2xl ring-1 transition
                                        {{ (int) $rateValue >= $i ? 'bg-amber-400/20 text-amber-600 ring-amber-300/60 dark:bg-amber-400/15 dark:text-amber-300 dark:ring-amber-300/30' : 'bg-slate-50 text-slate-300 ring-slate-200 hover:bg-slate-100 dark:bg-slate-950/30 dark:text-slate-600 dark:ring-white/10 dark:hover:bg-slate-800' }}">
                            ...
                            @error('rateValue')
                                <div class="mt-2 text-xs font-semibold text-rose-600 dark:text-rose-300">{{ $message }}</div>
                            @enderror
                        </div>

                        <div>
                            <div class="text-sm font-extrabold text-slate-900 dark:text-white">Comment (optional)</div>
                            <textarea wire:model.defer="rateComment" rows="4"
                                class="mt-3 w-full rounded-2xl bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-900 ring-1 ring-slate-200 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-200 dark:bg-slate-950/30 dark:text-white dark:ring-white/10 dark:focus:ring-indigo-800/50"
                        ...
                        <div class="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <button type="button" wire:click="saveRating" wire:loading.attr="disabled" wire:target="saveRating"
